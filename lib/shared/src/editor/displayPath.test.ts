...
describe('displayPathDirname', () => {
    ...
    describe('nonWindows', () => {
        test('no workspace folders', () => {
            ...
            expect(displayPathDirnameWithEnvInfo(URI.file('/foo/bar.ts'), envInfo)).toBe('/foo')
        })

        test('1 workspace folder', () => {
            const envInfo: DisplayPathEnvInfo = { isWindows: false, workspaceFolders: [URI.file('/workspace')] }
            expect(displayPathDirnameWithEnvInfo(URI.file('/workspace/foo.ts'), envInfo)).toBe('.')
            expect(displayPathDirnameWithEnvInfo(URI.file('/workspace/foo/bar.ts'), envInfo)).toBe('foo')
            expect(displayPathDirnameWithEnvInfo(URI.file('/other/foo.ts'), envInfo)).toBe('/other')
        ...
        test('2 workspace folders', () => {
            ...
            expect(displayPathDirnameWithEnvInfo(URI.file('/workspace1/foo.ts'), envInfo)).toBe('workspace1')
            expect(displayPathDirnameWithEnvInfo(URI.file('/workspace2/foo.ts'), envInfo)).toBe('workspace2')
            expect(displayPathDirnameWithEnvInfo(URI.file('/workspace1/foo/bar.ts'), envInfo)).toBe('workspace1/foo')
            expect(displayPathDirnameWithEnvInfo(URI.file('/workspace2/foo/bar.ts'), envInfo)).toBe('workspace2/foo')
            expect(displayPathDirnameWithEnvInfo(URI.file('/other/foo.ts'), envInfo)).toBe('/other')
            expect(displayPathDirnameWithEnvInfo(URI.parse('https://example.com/a/b'), envInfo)).toBe(
                'https://example.com/a'
            )
...
describe('displayPathBasename', () => {
    ...
    test('nonWindows', () => {
        const envInfo: DisplayPathEnvInfo = { isWindows: false, workspaceFolders: [] }
        expect(displayPathBasenameWithEnvInfo(URI.file('/foo/bar.ts'), envInfo)).toBe('bar.ts')
