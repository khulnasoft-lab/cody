...
import { displayPath, displayPathWithoutWorkspaceFolderPrefix } from '../../editor/displayPath'
    ...
    private async displaySearchResults(
        ...
        for (const file of resultContext.results) {
            ...
            let uri: string = new URL(
                `/search?q=context:global+file:${displayPathWithoutWorkspaceFolderPrefix(file.uri)}`,
                endpointUri
            ).href
